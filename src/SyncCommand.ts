...
export class SyncCommand implements ICommand {
  async execute(): Promise<void> {
    ...
    const manifest = await this.generateModel(routes);
    writeManifest(manifest);
  }

  async generateModel(routes: IConfiguredRoute[]): Promise<TrigintaManifest> {
    ...
    //TRIGINTA_MANIFEST_PATH
    let { name } = require(path.join(CWD, './package.json')) as { name: string };
    if (fs.existsSync(TRIGINTA_MANIFEST_PATH)) {
      ({ name } = parseManifest());
    }
