...
class ScriptTransformer {
  ...
  constructor(
    ...
    if (!projectCache) {
      projectCache = {
        configString,
        ignorePatternsRegExp: undefined,
  ...
  private _createCachedFilename(filename: string, cacheKey: string): string {
    const HasteMapClass = HasteMap.getStatic(this._config as Config.ProjectConfig);
    const baseCacheDir = HasteMapClass.getCacheFilePath(
      this._config.cacheDirectory,
      `triginta-transform-cache`,
      VERSION,
    );
  ...
  transform(
    ...
    const scriptCacheKey = getScriptCacheKey(filename, false);
